...
async function run() {
  ...
  const targetUnit = 'escola pequeno liceu';
  const activeStatuses = ['ativo', 'ativa', 'active'];
  let count = 0;
  ...
  for (const aluno of context.alunos) {
      ...
      for (const matricula of context.matriculas) {
          if (matricula.aluno_id === aluno.id &&
              matricula.unidade && matricula.unidade.toLowerCase() === targetUnit &&
              matricula.status && activeStatuses.includes(matricula.status.toLowerCase())) {
              matched = true;
              break;
          }
      }
