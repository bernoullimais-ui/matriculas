...
// Setup Supabase (initialized lazily to avoid ESM import hoisting issues with dotenv)
let supabase: any;

const superNormalize = (t: any) => 
  String(t || '').toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, '');

export async function syncWixRecurringPayments() {
  if (!supabase) {
        ...
        if (responsavel && email) {
          ...
          if (students.length > 0) {
            ...
            if (preAssignedStudentId) {
              ...
              const active = studentEnrollments.filter(e => e.status === 'ativo' || e.status === 'Ativo');
            ...
            if (!selectedEnrollment && candidates.length > 0) {
              ...
              const active = searchPool.filter(e => e.status === 'ativo' || e.status === 'Ativo');
