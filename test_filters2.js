...
const pDates = ["02/06/2026", "25/03/2026,", "29/03/2026,"];
const start = parseToDate("2026-06-01");
const end = parseToDate("2026-06-30");

const filtered = pDates.filter(d => {
    const pDate = parseToDate(d);
    if (start && pDate && pDate < start) return false;
    if (end && pDate && pDate > end) return false;
    return true;
});
