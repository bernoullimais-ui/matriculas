...
import { 
  Plus, Edit2, Trash2, Save, Tag, Ticket, X, BookOpen
} from 'lucide-react';
...
export function CuponsTab() {
  ...
  const [cupomForm, setCupomForm] = useState<any>({
    ...
    aplicabilidade: 'todos', produto_id: '', evento_id: '',
  ...
  const [cupomCursoForm, setCupomCursoForm] = useState<any>({
    ...
    data_expiracao: '', limite_uso: '', limite_por_usuario: '', uso_unico_cliente: false, ativo: true
  });
  ...
  const handleCouponCursoSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const token = sessionStorage.getItem('admin_token');
    const headers = { 'Content-Type': 'application/json', ...(token ? { 'Authorization': `Bearer ${token}` } : {}) };
    ...
    const payload = {
      ...cupomCursoForm,
      limite_uso: cupomCursoForm.limite_uso !== '' ? parseInt(cupomCursoForm.limite_uso) : null,
      limite_por_usuario: cupomCursoForm.limite_por_usuario !== '' ? parseInt(cupomCursoForm.limite_por_usuario) : null,
      data_expiracao: cupomCursoForm.data_expiracao || null
    };
    ...
    try {
      ...
      if (res.ok) {
        toast.success('Cupom de curso criado com sucesso!');
        setCupomCursoForm({
          codigo: '', tipo: 'porcentagem', valor: 0, data_inicio: getTodayDateString(),
          data_expiracao: '', limite_uso: '', limite_por_usuario: '', uso_unico_cliente: false, ativo: true
        });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-4 border-b border-slate-200">
        ...
        <button 
          onClick={() => {
            setCuponsSubTab('eventos');
            setCupomForm(prev => ({ ...prev, escopo: 'eventos_todos', produto_id: '', evento_id: '' }));
          }}
          className={`px-4 py-3 font-bold border-b-2 transition-colors ${cuponsSubTab === 'eventos' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
        >
          <span className="flex items-center gap-2"><Ticket size={18}/> Inscrições (Eventos)</span>
        </button>
        <button 
          onClick={() => {
            setCuponsSubTab('produtos');
            setCupomForm(prev => ({ ...prev, escopo: 'loja_todos', produto_id: '', evento_id: '' }));
          }}
      ...
      {cuponsSubTab === 'cursos' && (
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            ...
            <form onSubmit={handleCouponCursoSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Código do Cupom</label>
                  <input type="text" required value={cupomCursoForm.codigo} onChange={(e) => setCupomCursoForm({...cupomCursoForm, codigo: e.target.value})} className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none uppercase font-bold" placeholder="EX: MATRICULA2025"/>
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-2">Tipo de Desconto</label>
                  <select value={cupomCursoForm.tipo} onChange={(e) => setCupomCursoForm({...cupomCursoForm, tipo: e.target.value})} className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none">
                ...
                <div>
                  ...
                  <input type="number" required min="0" step="0.01" value={cupomCursoForm.valor} onChange={(e) => setCupomCursoForm({...cupomCursoForm, valor: parseFloat(e.target.value)})} className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none"/>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {cuponsDeMatriculas.length === 0 ? (
              <div className="col-span-full py-12 text-center text-slate-500 bg-slate-50 rounded-2xl border border-slate-100 border-dashed">
                Nenhum cupom de curso encontrado.
              </div>
      ...
      {cuponsSubTab === 'eventos' && (
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
            <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center gap-2">
              <Plus size={20} className="text-indigo-600"/> 
              Novo Cupom de Evento
            </h3>
            <form onSubmit={handleCouponSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={cupomForm.escopo} onChange={(e) => setCupomForm({...cupomForm, escopo: e.target.value, evento_id: ''})} className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none mb-2">
                    <option value="eventos_todos">Todos os Eventos</option>
                    <option value="eventos_especifico">Evento Específico</option>
                  </select>
                  {cupomForm.escopo === 'eventos_especifico' && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {cupons.filter(c => c.escopo === 'eventos_todos' || c.escopo === 'eventos_especifico').length === 0 ? (
              <div className="col-span-full py-12 text-center text-slate-500 bg-slate-50 rounded-2xl border border-slate-100 border-dashed">
                Nenhum cupom de evento encontrado.
              </div>
            ) : cupons.filter(c => c.escopo === 'eventos_todos' || c.escopo === 'eventos_especifico').map(cupom => (
              <div key={cupom.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative group overflow-hidden">
      ...
      {cuponsSubTab === 'produtos' && (
        <div className="space-y-6">
            ...
            <form onSubmit={handleCouponSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select value={cupomForm.escopo} onChange={(e) => setCupomForm({...cupomForm, escopo: e.target.value, produto_id: ''})} className="w-full border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 outline-none mb-2">
                    <option value="loja_todos">Aplicar a todos os produtos</option>
                    <option value="loja_especifico">Produto Específico</option>
                  </select>
                  {cupomForm.escopo === 'loja_especifico' && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {cupons.filter(c => c.escopo === 'loja_todos' || c.escopo === 'loja_especifico').length === 0 ? (
              <div className="col-span-full py-12 text-center text-slate-500 bg-slate-50 rounded-2xl border border-slate-100 border-dashed">
                Nenhum cupom da loja encontrado.
              </div>
            ) : cupons.filter(c => c.escopo === 'loja_todos' || c.escopo === 'loja_especifico').map(cupom => (
              <div key={cupom.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm relative group overflow-hidden">
